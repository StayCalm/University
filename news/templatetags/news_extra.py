from django import template
from django.utils.safestring import mark_safe

import mistune

register = template.Library()

@register.inclusion_tag('news/_item_tag.html', takes_context=True)
def news_item(context, item, **kwargs):
    kwargs['item'] = item
    kwargs['request_user'] = context['user']
    return kwargs

@register.inclusion_tag('news/_link_user_tag.html')
def link_user(user):
    return {
        'user': user
    }

@register.simple_tag
def user_arrows(user, item):
    if user == item.user:
        return ['star']
    else:
        res = []
        if item.can_be_upvoted_by(user=user):
            res.append('up')
        if item.can_be_downvoted_by(user=user):
            res.append('down')
        return res

from ..models import Story
import re
from ratelimit.decorators import ratelimit
#@register.inclusion_tag('news/_more_link_tag.html', takes_context=False)
#@ratelimit(key="user_or_ip", group="news-get",  method=['GET'], block=True)
#@ratelimit(key="user_or_ip", group="news-post",  method=['POST'], block=True)
def more_link1():#context):
    #request = context.request
    #page = int(request.GET.get('p', 0))
    #query_dict = request.GET.copy()
    #query_dict['p'] = page
    f = open('/Users/amalakhova/Documents/news_site/parser/events/' + 'News3' + '.txt', "r")
    all_text = ''
    all_arr_clast = []
    all_texts_arr = []
    f1 = f.readlines()
    for x in f1:
        all_text.join(x)
    all_arr_clast = re.findall(r'ioioioioiiioioi(\d*?)okokokokokkokokokokok', str(f1))
    #print(all_arr_clast)
    all_texts_arr = re.findall(r'okokokokokkokokokokok(.*?)plplplplplpplplplp', str(f1))
    i = 0
    while(i <len(all_arr_clast)):
        event1 = Story(title=all_arr_clast[i], text=all_texts_arr[i], user_id='5fab9d6c15ff40798e6d5b3d982db1f1')
        event1.save()
        i += 1
    f.close()
    #_more_link=request.path_info + '?' + query_dict.urlencode()
    #return {'more_link': _more_link}

@register.inclusion_tag('news/_more_link_tag1.html', takes_context=True)
def more_link(context):
    request = context.request
    page = int(request.GET.get('p', 0))
    query_dict = request.GET.copy()
    query_dict['p'] = page + 1
    _more_link=request.path_info + '?' + query_dict.urlencode()
    return {'more_link': _more_link}


@register.inclusion_tag('news/_item_content_tag.html')
def item_content(**kwargs):
    return kwargs


@register.inclusion_tag('news/_item_control_tag.html')
def item_control(**kwargs):
    return kwargs


class MarkdownRenderer(mistune.Renderer):

    @classmethod
    def setup(cls):
        renderer = cls(escape=True, hard_wrap=True)
        return mistune.Markdown(renderer=renderer)

    def header(self, text, level, raw=None):
        level = min(level + 2, 6)
        return super().header(text, level, raw)


markdown = MarkdownRenderer.setup()

@register.filter
def comment_markdown(value):
    return mark_safe(markdown(value))
